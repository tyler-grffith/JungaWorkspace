...
Everything lives on one branch, `main`, in three places: this checkout, GitHub, and `tylergriffith.us`, which vendors this app's `dist/` output as plain static files under `JungaWorkspace/` with no build step on the server. One command lines all three up:

```
npm run status                       # where do local, GitHub, the website repo and the live site stand?
npm run ship -- -m "What changed"    # commit, fast-forward, check, push, wait for CI, deploy live, verify
npm run ship -- --no-deploy          # stop after GitHub
```

`ship` refuses to go on when the tree is dirty with no message, when a rebase against `origin/main` conflicts, when `npm run check` fails, or when CI is red or cancelled. The website half on its own is `npm run deploy` (`--push`, `--live`); it assumes the website repo is a sibling directory (`../tylergriffith.us`), override with `WEBSITE_REPO`.

**[docs/RELEASING.md](docs/RELEASING.md) is the full runbook** — what each stage does and does not change, the PR path when a change wants review first, and the failure modes worth recognizing. Per [AGENTS.md](AGENTS.md), deploying is Tyler's explicit decision, never a side effect of a feature change: `ship` runs only when he asks for it.
...
The **Examples** view in the sidebar holds one built-in project for every kind of project: LaPlace Intuition, XJ-1 Flight Envelope, and Octahedron Sections (Graph) (graphing), Compressible Flow Calculator (spreadsheet), Octahedron Sections (spreadsheet + graph), Orbit Sketch (code files with a run output), Cosmic Clock (bundled source with the Earth Clock scene), PDR Visuals (visual canvas, imported from the original draw.io file), Pre-Development Plan (document), Workspace Ideas (collection), Mounting Bracket (3D modeler + slicer), and Sunset Relief (PLA Painter + slicer). They exist in every library whatever is saved: edit one freely (the edit persists), **Reset to original** brings it back, and **Duplicate** makes a copy of your own; they cannot be archived or trashed. **Shortcuts** in the sidebar link straight to chosen examples (Cosmic Clock and Octahedron Sections by default; the list is a designer setting).

Open any graphing project's calculator to edit functions, add sliders/constants and notes, change curve labels/colors, and pan or zoom the graph. Expressions support arithmetic, named functions/dependencies, common real math functions, and domain restrictions such as `f(t) = e^(-t) sin(t) {t > 0}`. Use **How to write expressions** for notation details. The LaPlace Intuition example recreates the first Desmos example with five curves and two sliders; XJ-1 Flight Envelope recreates the advanced aircraft model.
...
Open a spreadsheet project to edit cells and formulas, select ranges, copy/paste tables, fill relative references, format cells, and resize columns. Use **Load motion example** for editable inputs and a calculated velocity/distance table, or open the Compressible Flow Calculator example from the **Examples** menu. Arithmetic, A1 ranges, absolute/mixed references, and common aggregate/logical/engineering functions are supported; **Using the spreadsheet** lists the syntax. The first version has one worksheet per project, starting at 50 rows × 12 columns and growing to 200 × 26.

Projects containing both tools offer **Open side by side**. **Octahedron Sections** under Shortcuts opens the first integrated example: six coordinate pairs driven by s = 5, h = SQRT(3)*s/2, and a t slider from 0 to 1. Change B2 or move t to update the spreadsheet and connected shape together. **Link settings** configures named cells, numeric cell sliders, and x/y ranges for point series. Formula errors remove affected points and break lines. The graph uses equal axis scale for linked geometry unless **Equal axes** is switched off; **Fit points** adjusts its bounds.

Graph and spreadsheet documents save with the project and are included in library backups. Point-series links reference the same project's sheet; graph expressions cannot yet use sheet names. Undo/redo is session-local; slider/settings changes currently reset spreadsheet undo history. Spreadsheet cells commit on Enter, Tab, or blur; Escape cancels. Save failures keep a draft and guard navigation. Multiple worksheets, cross-sheet/custom functions, external imports, graph formatted math, shaded inequalities, and calculus remain deferred. Numeric curve plotting is approximate and can miss very narrow or rapidly oscillating features. Implicit contour sampling can also miss repeated-root curves and isolated roots; use a simpler equivalent equation or a closer view. **Equal axes** is on by default for points and implicit geometry and off otherwise; switch it per graph when x and y span very different ranges, such as speed against power.
