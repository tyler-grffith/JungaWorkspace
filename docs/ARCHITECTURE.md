...
| Examples        | `src/modules/examples.ts`                                                                                                                                | Built-in example projects, one per kind of project, merged into every library by `builtins.ts`.                                            |
...
Storage, validation on read, backup drafts, duplication, the editor route, the outputs panel, the output route, and library-card art all follow from the registry. Canvas, document, collection, modeler, slicer, and painter are built this way. Desktop-application-shaped modules (the modeler and slicer) describe their menus, command tabs, and tools as data and render them through `src/workbench/Workbench.tsx`, so a new tool is a spec entry with typed parameters rather than a hand-built panel.
...
Examples are _built-in projects_: one per kind of project (every module, the spreadsheet + graph pairing, and the bundled Cosmic Clock code project), present in every library whatever the user has saved. `src/modules/builtins.ts` builds the pristine projects from the registry once per session (ids `example-<id>`, stable output ids, a fixed date), and `useLibrary` merges them over the stored library on every read and strips the unchanged ones on every write. A built-in the user edits is stored as an _overlay_ under the same id, so the edit survives reloads and rides along in backups; **Reset to original** drops the overlay. Built-ins cannot be archived or trashed (the library refuses), live in the **Examples** view rather than **All projects**, and duplicate into ordinary projects. **Shortcuts** are the built-ins named by the `library.shortcuts` design setting (a comma-separated list of example ids), linked from the sidebar straight to their opening route.
...
- Browser tests reference visible labels (`Open calculator`, `Open this example`, the `Examples` view, the `Shortcuts` links such as `Cosmic Clock`, designer field labels). Changing a label means updating the test or the registry that the test reads.
