...
**3D modeler.** The viewport is a real orthographic renderer: drag to orbit, shift-drag or right-drag to pan, wheel to zoom, double-click or **F** to fit, arrow keys to orbit; the heads-up toolbar's Isometric/Front/Top/Right and Zoom to Fit work. Sketches take an **Offset from plane** (defaulting to the top of the material) and a centre; extrude, revolve (a centred rectangle becomes a cylinder, an off-centre one a ring), cut, hole (from the top face, with **Through All**), mirror, linear pattern, and reference planes produce real meshes; fillet, chamfer, and shell are recorded and badged "awaits kernel". Mass properties are signed mesh volumes. **File › Export STL…** writes a binary STL; **Evaluate › Send to Slicer** (or File) places the part on the project's slicer plate and opens it, when the project has both tools.

**Slicer.** Objects are boxes, imported meshes (**Prepare › Import STL** or File), or parts from the modeler (**File › Import from Modeler**). Drag an object in the viewport to move it on the plate; rotation and scale are in the object panel. **Slice plate** really slices: Preview draws outer walls, inner walls, sparse infill, solid infill, and brim per layer (legend under the estimate), and **File › Export Plate as G-code…** writes the same paths as G-code. Time and filament come from those paths; supports are not generated and the printer stays simulated.

**Validation.** 154 unit tests, 77 Playwright tests (three in `tests/workbench.spec.ts`, including the modeler-to-slicer round trip), `npm run check`. Decisions 45–46.
