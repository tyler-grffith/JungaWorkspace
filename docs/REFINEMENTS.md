...
Areas: **Label manager** (control variants, rotation step, popup geometry, with a sample using the real label manager), **Graph layout**, **Spreadsheet** (default column width, row height, cell text size), **Visual canvas** (page strip and inspector widths, default grid size, default fill and stroke, selection color, rulers), **Shared appearance** (accent, sidebar, text, border and soft colors; button and card corners; base text size; heading font), **Workspace shell** (sidebar width, content width and padding, brand and workspace text), and **Project library** (grid columns, default view and sort, example buttons, library copy). Every control is one entry in [src/design/registry.ts](../src/design/registry.ts); see [ARCHITECTURE.md](ARCHITECTURE.md) for adding more.
