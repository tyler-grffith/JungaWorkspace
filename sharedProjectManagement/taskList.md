...
- [ ] **Review the built-in examples.** Open **Examples** in the sidebar: every example is always there, edits to one persist, **Reset to original** discards them, and **Duplicate** gives a plain copy; **Shortcuts** links straight to Cosmic Clock and Octahedron Sections (the list is the `Shortcut examples` designer setting). Say which example is furthest from the original it recreates; the XJ-1 graph, the flow calculator sheet, the PDR board import, and the plan document are all one file each under `src/examples/`.
- [ ] **Review the modeler and slicer.** Make one project with both tools. Modeler: Corner Rectangle → Extruded Boss/Base, a second sketch (its offset defaults to the top face) → extrude, Hole Wizard, Revolved Boss/Base on a centred rectangle, Mirror, Linear Pattern; orbit by dragging, try the standard views and Zoom to Fit, Mass Properties, Export STL, then **Send to Slicer**. Slicer: drag the part around the plate, Import STL with any file you have, Slice plate, scrub layers in Preview and read the legend, Export Plate as G-code and open it in a text editor. Note which SolidWorks or Bambu Studio behaviour you miss most.
- [ ] **Review PLA Painter.** Make a project with PLA Painter and Slicer. Drop in a photo, watch the printed preview, move the swap layers and change transmission distances, try Space evenly, look at the heightmap and 3D relief, export the STL and the print sheet, then Send to Slicer and slice it. Say whether the default stack (black, red, yellow, white) and the approximate TDs match your spools, and whether you want dithering or automatic swap placement next.
- [ ] **Decide on supports, a sketcher, and a kernel.** The slicer generates no supports yet (the estimate says so); the modeler has no free sketching and no fillet/chamfer/shell geometry. Each is a bounded next increment on the mesh layer; a boundary-representation kernel would be a larger decision.
- [ ] **Decide where meshes live.** Imported meshes are stored inside the slicer document (decimated to 4,000 triangles, 2 MB per document) because the library shares one local-storage key. An IndexedDB asset store (also wanted for canvas and collection images) would lift both limits.
...
- [ ] **Revisit the advanced aircraft graph.** XJ-1 Flight Envelope is now a built-in example: constants are constants, s is the one reader slider, and the reference's hidden label-anchor points became curve labels and notes. Still open: a derivative operator (the best-climb line is an analytic derivative), folders for author parameters, and labels pinned to plot lines.
...
## Revision pass and real geometry — September 24, 2026

- [x] Lazy module editors and viewers; deferred library writes with flush; `related` handle on `EditorProps`; cached canvas text measurement; memoized thumbnails; single-pass collection counts; clipboard images in and out; Junga project links in collections (decision 45).
- [x] `src/workbench/geometry.ts` (profiles, extrude, revolve, transforms, volume, STL, slicing, offsets, hatching) and `src/workbench/Viewport3D.tsx` (orbit/pan/zoom/fit, picking, dragging, hard edges) replace the isometric SVG (decision 46).
- [x] Modeler: real rebuild of extrude/revolve/cut/hole/mirror/pattern/plane, sketch offset and position, mass from mesh volume, STL export, Send to Slicer.
- [x] Slicer: mesh objects, STL import with decimation, Import from Modeler, drag on the plate, rotation and scale, real slicing with walls/infill/solid/brim, layer preview by line type, G-code and STL export.
...
- [x] `collection` added as the sixth module: a graph of nested collections over a shared item pool (children and items may have several parents; loops refused), per-collection fields with presets, quick add from a title or link with YouTube covers, in-place editing, grid/list/gallery layouts, search, a browse output, Markdown/JSON export and JSON import, a designer-mode group.
- [x] Document-module registry (`src/modules/documents.ts`, `src/modules/editors.tsx`) replaces per-module wiring in the shell; canvas and document moved onto it. New modules of this shape are two registry entries plus their folder.
- [x] Unit tests for the model and library wiring; two Chromium tests. Same branch `codex/canvas-module`; no merge or deployment.
- [ ] Awaiting Tyler: drag-and-drop filing, CSV/Spotify import, and link metadata fetching (needs a server) are the natural next steps.
